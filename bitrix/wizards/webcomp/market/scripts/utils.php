...
{
    static function PatchHtaccess($path)
    {
        if (mb_strtoupper(mb_substr(PHP_OS, 0, 3)) === "WIN") {
            ...
            if ($io->FileExists($fnhtaccess)) {
                ...
                if (mb_strpos($ffhtaccessContent,
                        "/bitrix/virtual_file_system.php") === false
                ) {
                    $ffhtaccessContent = preg_replace('/RewriteEngine On/is',
                        "RewriteEngine On\r\n\r\n".
    ...
    static function GetTemplates($relativePath)
    {
        ...
        while (($dirName = @readdir($handle)) !== false) {
            ...
            if (file_exists($absolutePath."/".$dirName."/lang/".LANGUAGE_ID
                ."/screen.png")
            ) {
                $arTemplate["SCREENSHOT"] = $relativePath."/".$dirName."/lang/"
                    .LANGUAGE_ID."/screen.png";
            } elseif (file_exists($absolutePath."/".$dirName."/screen.png")) {
                $arTemplate["SCREENSHOT"] = $relativePath."/".$dirName
                    ."/screen.png";
            } else {
            ...
            if (file_exists($absolutePath."/".$dirName."/lang/".LANGUAGE_ID
                ...
                $arTemplate["PREVIEW"] = $relativePath."/".$dirName."/lang/"
                    .LANGUAGE_ID."/preview.png";
            } elseif (file_exists($absolutePath."/".$dirName."/preview.png")) {
                $arTemplate["PREVIEW"] = $relativePath."/".$dirName
                    ."/preview.png";
            } else {
        ...
        uasort($arWizardTemplates, 'sortFn');
    ...
    static function GetServices($wizardPath, $serviceFolder = "", $arFilter = [])
    {
        $arServices = [];
        ...
        if (LANGUAGE_ID != "en" && LANGUAGE_ID != "ru") {
            if (file_exists(($fname = $wizardPath."/lang/"
                .LangSubst(LANGUAGE_ID).$serviceFolder."/.services.php"))
            ) {
                __IncludeLang($fname);
            }
        }
        if (file_exists(($fname = $wizardPath."/lang/".LANGUAGE_ID
            .$serviceFolder."/.services.php"))
        ) {
            __IncludeLang($fname);
        }
        ...
        foreach ($arServices as $serviceID => $arService) {
            if (isset($arFilter["SKIP_INSTALL_ONLY"])
                && array_key_exists("INSTALL_ONLY", $arService)
                && $arService["INSTALL_ONLY"] == $arFilter["SKIP_INSTALL_ONLY"]
            ) {
                unset($arServices[$serviceID]);
                continue;
            }

            if (isset($arFilter["SERVICES"]) && is_array($arFilter["SERVICES"])
                && ! in_array($serviceID, $arFilter["SERVICES"])
                && ! array_key_exists("INSTALL_ONLY", $arService)
            ) {
                unset($arServices[$serviceID]);
                continue;
            ...
            $modulesCheck = [$serviceID];
            if (array_key_exists("MODULE_ID", $arService)) {
                $modulesCheck = (is_array($arService["MODULE_ID"])
                    ? $arService["MODULE_ID"] : [$arService["MODULE_ID"]]);
            }
            ...
            $servicePosition += (isset($arService["STAGES"])
            && ! empty($arService["STAGES"]) ? count($arService["STAGES"]) : 1);
        }

        return $arServices;
    }

    static function IncludeServiceLang(
        $relativePath,
        $lang = false,
        $bReturnArray = false
    ) {
        ...
        if ($lang != "en" && $lang != "ru") {
            ...
            $fname = WIZARD_SERVICE_ABSOLUTE_PATH."/lang/".$subst_lang."/"
                .$relativePath;
            $fname
                = \Bitrix\Main\Localization\Translation::convertLangPath($fname,
                $subst_lang);
        ...
        $fname = \Bitrix\Main\Localization\Translation::convertLangPath($fname,
            $lang);
        if (file_exists($fname)) {
            if ($bReturnArray) {
                $arMessages = array_merge($arMessages,
                    __IncludeLang($fname, true, true));
            } else {
                __IncludeLang($fname, false, true);
            }
        }
    ...
    static function GetCurrentSiteID($selectedSiteID = null)
    {
        if ($selectedSiteID <> '') {
        ...
        if ($handle = @opendir($themePath)) {
            while (($file = readdir($handle)) !== false) {
                ...
                if (is_file($themePath."/".$file."/description.php")) {
                    if (LANGUAGE_ID != "en" && LANGUAGE_ID != "ru") {
                        if (file_exists(($fname = $themePath."/".$file."/lang/"
                            .LangSubst(LANGUAGE_ID)."/description.php"))
                        ) {
                            __IncludeLang($fname);
                        }
                    }
                ...
                $arTheme = [
                    "ID"   => $file,
                    "SORT" => (isset($arTemplate["SORT"])
                    && intval($arTemplate["SORT"]) > 0
                        ? intval($arTemplate["SORT"]) : 10),
                    "NAME" => (isset($arTemplate["NAME"]) ? $arTemplate["NAME"]
                        : $file),
                ];

                if (file_exists($themePath."/".$file."/lang/".LANGUAGE_ID
                    ."/small.png")
                ) {
                    $arTheme["PREVIEW"] = $relativePath."/".$file."/lang/"
                        .LANGUAGE_ID."/small.png";
                } elseif (file_exists($themePath."/".$file."/lang/".LANGUAGE_ID
                    ."/preview.png")
                ) {
                    $arTheme["PREVIEW"] = $relativePath."/".$file."/lang/"
                        .LANGUAGE_ID."/preview.png";
                ...
                } elseif (file_exists($themePath."/".$file."/preview.png")) {
                    $arTheme["PREVIEW"] = $relativePath."/".$file
                        ."/preview.png";
                } else {
                ...
                if (file_exists($themePath."/".$file."/lang/".LANGUAGE_ID
                    ...
                    $arTheme["SCREENSHOT"] = $relativePath."/".$file."/lang/"
                        .LANGUAGE_ID."/big.png";
                } elseif (file_exists($themePath."/".$file."/lang/".LANGUAGE_ID
                    ."/screen.png")
                ) {
                ...
                } elseif (file_exists($themePath."/".$file."/screen.png")) {
                    $arTheme["SCREENSHOT"] = $relativePath."/".$file
                        ."/screen.png";
                } else {
    ...
    static function SetFilePermission($path, $permissions)
    {
        ...
        return $GLOBALS["APPLICATION"]->SetFileAccessPermission($originalPath,
            $arPermisson);
    }

    static function AddMenuItem($menuFile, $menuItem, $siteID, $pos = -1)
    {
        if (CModule::IncludeModule('fileman')) {
            $arResult = CFileMan::GetMenuArray($_SERVER["DOCUMENT_ROOT"]
                .$menuFile);
            ...
            if ( ! $bFound) {
                ...
                CFileMan::SaveMenu([$siteID, $menuFile], $arMenuItems,
                    $menuTemplate);
            }
        }
    }

    static function CopyFile($fileFrom, $fileTo)
    {
        CopyDirFiles($_SERVER['DOCUMENT_ROOT'].$fileFrom,
            $_SERVER['DOCUMENT_ROOT'].$fileTo, false, true);
    }

    static function ImportIBlockFromXML(
        $xmlFile,
        $iblockCode,
        $iblockType,
        $siteID,
        ...
        require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/iblock/classes/"
            .mb_strtolower($GLOBALS["DB"]->type)."/cml2.php");
    ...
    static function SetIBlockFormSettings($iblockID, $settings)
    {
        global $DBType;
        require_once($_SERVER['DOCUMENT_ROOT']."/bitrix/modules/main/classes/"
            .mb_strtolower($DBType)."/favorites.php");
    ...
    static function CreateSectionProperty($iblockID, $fieldCode, $arFieldName = [])
    {
        $entityID = "IBLOCK_".$iblockID."_SECTION";

        $dbField = CUserTypeEntity::GetList([],
            ["ENTITY_ID" => $entityID, "FIELD_NAME" => $fieldCode]);
        ...
        $GLOBALS["USER_FIELD_MANAGER"]->arFieldsCache = [];
