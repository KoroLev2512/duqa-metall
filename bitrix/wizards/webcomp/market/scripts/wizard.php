...
class CSelectSiteWizardStep extends CWizardStep
{
    ...
    function OnPostForm()
    {
        ...
        if ($wizard->IsNextButtonClick()) {
            ...
            $siteFolder = str_replace(["\\", "///", "//"], "/",
                "/".$wizard->GetVar("siteFolder")."/");
            ...
            if ($createSite == "Y") {
                ...
                $db_res = CSite::GetList($by = "sort", $order = "desc",
                    ["LID" => $siteID]);
...
function SelectCreateSite(element, solutionId)
{
	...
	if (check)
        ...
        $this->content .= 'class="solution-item'.($createSite != "Y"
        ...
        if (count($arSites) < 2) {
            $this->content .= '<p>'.GetMessage("wiz_site_existing_title").' '
                .implode("", $arSitesSelect).'</p>';
        } else {
            $this->content .= '<p>'.GetMessage("wiz_site_existing_title");
            $this->content .= "<br />".$this->ShowSelectField("siteID",
                    $arSitesSelect)."</p>";
        }
        $this->content .= '</div>';
        $this->content .= '<b class="r1"></b><b class="r1"></b><b class="r3"></b>';
        $this->content .= '</div>';
        if (count($arSites) < COption::GetOptionInt("main", "PARAM_MAX_SITES",
                100)
            || COption::GetOptionInt("main", "PARAM_MAX_SITES", 100) <= 0
        ) {
            ...
            $this->content .= str_replace(
                ...
                [
                    $this->ShowInputField("text", "siteNewID",
                        ["size" => 2, "maxlength" => 2, "id" => "siteNewID"]),
                    $this->ShowInputField("text", "siteFolder",
                        ["id" => "siteFolder"]),
                ],
...
class CSelectTemplateWizardStep extends CWizardStep
{
    ...
    function OnPostForm()
    {
        ...
        if ($wizard->IsNextButtonClick()) {
            $templatesPath = WizardServices::GetTemplatesPath($wizard->GetPath()
                ."/site");
    ...
    function ShowStep()
    {
        ...
        if (isset($templateID) && array_key_exists($templateID, $arTemplates)) {
            ...
            $defaultTemplateID = COption::GetOptionString("main",
                "wizard_template_id", "", $wizard->GetVar("siteID"));
        ...
        foreach ($arTemplates as $templateID => $arTemplate) {
            ...
            if ($arTemplate["SCREENSHOT"] && $arTemplate["PREVIEW"]) {
                $this->content .= CFile::Show2Images($arTemplate["PREVIEW"],
                    $arTemplate["SCREENSHOT"], 150, 150,
                    ' class="inst-template-list-img"');
            } else {
                $this->content .= CFile::ShowImage($arTemplate["SCREENSHOT"],
                    150, 150, ' class="inst-template-list-img"', "", true);
            }

            $this->content .= '<label for="'.$templateID
                .'" class="inst-template-list-label">'.$arTemplate["NAME"].'<p>'
                .$arTemplate["DESCRIPTION"].'</p></label>';
            $this->content .= "</div>";
...
class CSelectThemeWizardStep extends CWizardStep
{
    ...
    function OnPostForm()
    {
        ...
        if ($wizard->IsNextButtonClick()) {
            ...
            $templatesPath = WizardServices::GetTemplatesPath($wizard->GetPath()
                ."/site");
            $arThemes = WizardServices::GetThemes($templatesPath."/".$templateID
                ."/themes");
    ...
    function ShowStep()
    {
        ...
        if (isset($ThemeID) && array_key_exists($ThemeID, $arThemes)) {
            ...
            $defaultThemeID = COption::GetOptionString("main",
                "wizard_".$templateID."_theme_id", "",
                $wizard->GetVar("siteID"));

            if ( ! ($defaultThemeID <> ''
                && array_key_exists($defaultThemeID, $arThemes))
            ) {
                $defaultThemeID = COption::GetOptionString("main",
                    "wizard_".$templateID."_theme_id", "");
                if ($defaultThemeID <> ''
...
function SelectTheme(element, solutionId, imageUrl)
{
	...
	else 
	{
        ...
        foreach ($arThemes as $themeID => $arTheme) {
            ...
            $this->content .= '
					<span class="inst-template-color'.($defaultThemeID
                == $themeID ? " inst-template-color-selected" : "")
                .'" ondblclick="SubmitForm(\'next\');"  onclick="SelectTheme(this, \''
                .$themeID.'\', \''.$arTheme["SCREENSHOT"].'\');">
						<span class="inst-templ-color-img">'
                .CFile::ShowImage($arTheme["PREVIEW"], 70, 70,
                    ' border="0" class="solution-image"').'</span>
						<span class="inst-templ-color-name">'.$ii.'. '
                .$arTheme["NAME"].'</span>
					</span>';
        }

        $this->content .= $this->ShowHiddenField($themeVarName, $defaultThemeID,
            ["id" => "selected-solution"]);
        $this->content .= '</div>'.
            '<div id="solution-preview">'.
            '<b class="r3"></b><b class="r1"></b><b class="r1"></b>'.
            '<div class="solution-inner-item">'.
            CFile::ShowImage($arDefaultTheme["SCREENSHOT"], 450, 450,
                ' border="0" id="solution-preview-image"').
            '</div>'.
...
class CSiteSettingsWizardStep extends CWizardStep
{
    function InitStep()
    {
        ...
        if ($wizard->GetVar("createSite") == "Y") {
            ...
            $siteID
                = WizardServices::GetCurrentSiteID($wizard->GetVar("siteID"));
...
class CDataInstallWizardStep extends CWizardStep
{
    ...
    function ShowStep()
    {
        ...
        $arServices = WizardServices::GetServices($_SERVER["DOCUMENT_ROOT"]
            .$wizard->GetPath(), "/site/services/");

        [$firstService, $stage, $status] = $this->GetFirstStep($arServices);
			...
			<div id="error_container" style="display:none">
				<div id="error_notice">
					<div class="inst-note-block inst-note-block-red">
						...
						<div class="inst-note-block-label">'
            .GetMessage("INST_ERROR_OCCURED").'</div><br style="clear:both" />
						<div class="inst-note-block-text">'
            .GetMessage("INST_ERROR_NOTICE").'<div id="error_text"></div></div>
					</div>
				</div>

				<div id="error_buttons" align="center">
				<br /><input type="button" value="'
            .GetMessage("INST_RETRY_BUTTON")
            .'" id="error_retry_button" onclick="" class="instal-btn instal-btn-inp" />&nbsp;<input type="button" id="error_skip_button" value="'
            .GetMessage("INST_SKIP_BUTTON").'" onclick="" class="instal-btn instal-btn-inp" />&nbsp;</div>
			</div>
		...
		<script type="text/javascript">
			var ajaxForm = new CAjaxForm("'.$formName.'", "iframe-post-form", "'
            .$NextStepVarName.'");
    ...
    function GetFirstStep(&$arServices)
    {
        foreach ($arServices as $serviceID => $arService) {
            $stage = "index.php";
            if (array_key_exists("STAGES", $arService)
                && is_array($arService["STAGES"])
                && isset($arService["STAGES"][0])
            ) {
        ...
        return [
            "service_not_found",
            "finish",
            GetMessage("INSTALL_SERVICE_FINISH_STATUS"),
        ];
    }
    ...
    function OnPostForm()
    {
        ...
        if ($wizard->GetVar("siteCreate") == "Y") {
            if ( ! $res) {
                $culture = CultureTable::getRow([
                    'filter' => [
                        "=FORMAT_DATE"     => (LANGUAGE_ID == "en"
                            ? "MM/DD/YYYY" : "DD.MM.YYYY"),
                        "=FORMAT_DATETIME" => (LANGUAGE_ID == "en"
                            ? "MM/DD/YYYY H:MI:SS T" : "DD.MM.YYYY HH:MI:SS"),
                        "=FORMAT_NAME"     => CSite::GetDefaultNameFormat(),
                        "=CHARSET"         => (defined("BX_UTF")
                            ? "UTF-8"
                            : (LANGUAGE_ID == "ru" ? "windows-1251"
                                : "ISO-8859-1")),
                    ],
                ]);
                ...
                if ($culture) {
                    ...
                    $addResult = CultureTable::add([
                        "NAME"            => $site_id,
                        "CODE"            => $site_id,
                        "FORMAT_DATE"     => (LANGUAGE_ID == "en" ? "MM/DD/YYYY"
                            : "DD.MM.YYYY"),
                        "FORMAT_DATETIME" => (LANGUAGE_ID == "en"
                            ? "MM/DD/YYYY H:MI:SS T" : "DD.MM.YYYY HH:MI:SS"),
                        "FORMAT_NAME"     => CSite::GetDefaultNameFormat(),
                ...
                $arFields = [
                    ...
                    "EMAIL"       => COption::GetOptionString("main",
                        "email_from"),
        ...
        if ($res && (count($matches) > 0 || $res["NAME"] == $site_id)
            && $site_id != "s1"
        ) {
            $templateID = $wizard->GetVar("templateID");
            $themeVarName = $templateID."_themeID";
            ...
            $templatesPath = WizardServices::GetTemplatesPath($wizard->GetPath()
                ."/site");
            $arThemes = WizardServices::GetThemes($templatesPath."/".$templateID
                ."/themes", $templatesPath."/".$templateID);

            $siteNemNew = $defSiteName.": ".$arTemplates[$templateID]["NAME"]
                .' ('.$arThemes[$themeID]["NAME"].')';

            $obSite = new CSite;
            $obSite->Update($site_id,
                ["NAME" => $siteNemNew, "SITE_NAME" => $siteNemNew]);
            ...
            $obSite = new CSite;
            $obSite->Update($site_id,
                ["NAME" => $SiteNAME, "SITE_NAME" => $defSiteName]);
        }
        ...
        if ( ! $this->repeatCurrentService) {
            [$nextService, $nextServiceStage, $stepsComplete, $status]
                = $this->GetNextStep($arServices, $serviceID, $serviceStage);
        }

        if ($nextService == "finish") {
            $response
                = "window.ajaxForm.StopAjax(); window.ajaxForm.SetStatus('100'); window.ajaxForm.Post('"
                .$nextService."', '".$nextServiceStage."','".$status."');";
            COption::SetOptionString("main",
                "wizard_first".mb_substr($wizard->GetID(), 7)."_"
                .$wizard->GetVar("siteID"), "Y", false);
        } else {
            $arServiceID = array_keys($arServices);
            $lastService = array_pop($arServiceID);
            $stepsCount = $arServices[$lastService]["POSITION"];
            if (array_key_exists("STAGES", $arServices[$lastService])
                && is_array($arServices[$lastService])
            ) {
            ...
            $response = ($percent ? "window.ajaxForm.SetStatus('".$percent."');"
                    : "")." window.ajaxForm.Post('".$nextService."', '"
                .$nextServiceStage."','".$status."');";
        }
    ...
    function InstallService($serviceID, $serviceStage)
    {
        ...
        $templatesPath = WizardServices::GetTemplatesPath(WIZARD_RELATIVE_PATH
            ."/site");
        $templateID = $wizard->GetVar("templateID");

        define("WIZARD_TEMPLATE_ID", $templateID);
        define("WIZARD_TEMPLATE_RELATIVE_PATH",
            $templatesPath."/".WIZARD_TEMPLATE_ID);
        define("WIZARD_TEMPLATE_ABSOLUTE_PATH",
            $_SERVER["DOCUMENT_ROOT"].WIZARD_TEMPLATE_RELATIVE_PATH);

        $themeID = $wizard->GetVar($templateID."_themeID");
        define("WIZARD_THEME_ID", $themeID);
        define("WIZARD_THEME_RELATIVE_PATH",
            WIZARD_TEMPLATE_RELATIVE_PATH."/themes/".WIZARD_THEME_ID);
        define("WIZARD_THEME_ABSOLUTE_PATH",
            $_SERVER["DOCUMENT_ROOT"].WIZARD_THEME_RELATIVE_PATH);

        $servicePath = WIZARD_RELATIVE_PATH."/site/services/".$serviceID;
        define("WIZARD_SERVICE_RELATIVE_PATH", $servicePath);
        define("WIZARD_SERVICE_ABSOLUTE_PATH",
            $_SERVER["DOCUMENT_ROOT"].$servicePath);
        define("WIZARD_IS_RERUN", $_SERVER["PHP_SELF"] != "/index.php");

        define("WIZARD_SITE_LOGO", intval($wizard->GetVar("siteLogo")));
        define("WIZARD_INSTALL_DEMO_DATA",
            $wizard->GetVar("installDemoData") == "Y");
        define("WIZARD_REINSTALL_DATA", false);
        define("WIZARD_FIRST_INSTAL", COption::GetOptionString("main",
            "wizard_first".mb_substr($wizard->GetID(), 7)."_"
            .$wizard->GetVar("siteID"), false, $wizard->GetVar("siteID")));

        $dbUsers = CGroup::GetList($by = "id", $order = "asc",
            ["ACTIVE" => "Y"]);
        ...
        if ($langSubst <> LANGUAGE_ID) {
            if (file_exists(($fname = WIZARD_SERVICE_ABSOLUTE_PATH."/lang/"
                .$langSubst."/".$serviceStage))
            ) {
    ...
    function GetNextStep(&$arServices, &$currentService, &$currentStage)
    {
        ...
        if ($currentStage != "skip"
            && array_key_exists("STAGES", $arServices[$currentService])
            && is_array($arServices[$currentService]["STAGES"])
        ) {
            $stageIndex = array_search($currentStage,
                $arServices[$currentService]["STAGES"]);
            if ($stageIndex !== false
                && isset($arServices[$currentService]["STAGES"][$stageIndex
                    + 1])
            ) {
                return [
                    $currentService,
        ...
        if (array_key_exists("STAGES", $arServices[$nextServiceID])
            && is_array($arServices[$nextServiceID]["STAGES"])
...
class CFinishWizardStep extends CWizardStep
{
    ...
    function ShowStep()
    {
        ...
        $wizard->SetFormActionScript(str_replace("//", "/",
            $siteDir."/?finish"));

        $this->CreateNewIndex();

        COption::SetOptionString("main", "wizard_solution",
            $wizard->solutionName, false, $siteID);
