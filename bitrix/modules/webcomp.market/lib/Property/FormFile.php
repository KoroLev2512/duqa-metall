...
class FormFile {
    ...
    static function OnIBlockPropertyBuildList() {
      ...
      if(!isset($_REQUEST["propedit"]) && $_REQUEST["type"] !== self::IBLOCK_TYPE)
        ...
        return [
  	      ...
  	      "DESCRIPTION"           => Loc::getMessage('PROPERTY_FILE_NAME'),
    ...
    */
    static function ConvertToDB($arProperty, $value) {
        ...
        $arFile = CFile::MakeFileArray($_SERVER [ "DOCUMENT_ROOT" ].  '/upload/tmp'  . $value[ 'tmp_name' ]);
        $arFile['name'] = $value["name"];
    ...
    */
    static function GetPropertyFieldHtml($arProp, $value, $control) {
        ...
        if(empty($value["VALUE"])){
            ...
            $prop = str_replace('[VALUE]', '', $control["VALUE"]);
        }

        return  _ShowFilePropertyField($prop, $arProp, $value);
    }
    ...
    */
    ...
    static function PrepareSettings($arFields) {
      ...
      if(empty($arFields["USER_TYPE_SETTINGS"]["PROPERTY_MSG_ERROR_VALIDATE"])) {
        $arFields["USER_TYPE_SETTINGS"]["PROPERTY_MSG_ERROR_VALIDATE"] = Loc::getMessage("PROPERTY_FILE_MSG_ERROR_VALIDATE_DEFAULT");
      }
    ...
    static function GetSettingsHTML($arProp, $control, &$arPropertyFields) {
      ...
      $arPropertyFields = [
	      'HIDE' => [
	    	...
	    	'SET' => [],
	    	'SHOW' => [],
	    	'USER_TYPE_SETTINGS_TITLE' => Loc::getMessage("PROPERTY_FILE_CUSTOM_BLOCK")
      ];
    	...
    	<tr>
				<td width="40%" class="adm-detail-content-cell-l">
					<label for="PROPERTY_FILE_MSG_ERROR_VALIDATE"><?=Loc::getMessage("PROPERTY_FILE_MSG_ERROR_VALIDATE")?></label>
				</td>
				<td class="adm-detail-content-cell-r">
					<input  id="PROPERTY_FILE_MSG_ERROR_VALIDATE"
                  size="50" 
                  type="text" 
      ...
      <tr>
        <td width="40%" class="adm-detail-content-cell-l">
          <label for="PROPERTY_FILE_DISABLED_FIELD"><?=Loc::getMessage("PROPERTY_FILE_DISABLED_FIELD")?></label>
        </td>
        <td class="adm-detail-content-cell-r">
          <input id="PROPERTY_FILE_DISABLED_FIELD"
                type="checkbox" 
                value="Y" 
      ...
      <tr>
        <td width="40%" class="adm-detail-content-cell-l">
          <label for="PROPERTY_FILE_HIDDEN_FIELD"><?=Loc::getMessage("PROPERTY_FILE_HIDDEN_FIELD")?></label>
        </td>
        <td class="adm-detail-content-cell-r">
          <input id="PROPERTY_FILE_HIDDEN_FIELD"
                type="checkbox" 
                value="Y" 
