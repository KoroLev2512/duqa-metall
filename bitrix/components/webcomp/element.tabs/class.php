...
class ElementTabsComponent extends CBitrixComponent {
    private $APPLICATION;
    /**
     * @var int
     */
    ...
    public function onPrepareComponentParams($arParams)
    {
        ...
        if(empty($this->arParams["PROPERTY_ENUM"])) {
            return;
        }
    ...
    protected function selectElementsFromDB()
    {
        ...
        foreach ($this->arParams["PROPERTY_ENUM"] as $propertyID) {
            ...
            // Убираем дубли
            unset($arrFilter["PROPERTY_".$this->arParams["PROPERTY_ID"]."_VALUE"], $arrIDsToSelect);

            $arrFilter = array_merge($arrFilter, ["PROPERTY_".$this->arParams["PROPERTY_ID"]."_VALUE" => $rsEnum["VALUE"]]);
    ...
    protected function selectPropertiesAndAddToItems() {
        if(!empty($this->tabs)) {
            foreach ($this->tabs as $key => $tab) {
                while ($arItem = $tab["dbElements"]->Fetch()) {
                            ...
                            if ( ! empty($this->arProperties[$code])
                                ...
                                switch ($this->arProperties[$code]["PROPERTY_TYPE"]) {
                                    case "F":// File
                                        ...
                                        $this->arProperties[$code]['FILE_ID']
                                            = $fileId;
                                        break;
                                    /*case "L":// List
                    ...
                    $this->arResult[$key]['NAME'] = $tab["NAME"];
                    $this->arResult[$key]['ITEMS'][] = $arItem;
                }
            }
        }
    }
