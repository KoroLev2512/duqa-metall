	...
	if($folderPath <> '')
	{
		...
		if($obSearch->errorno==0)
		{
			...
			while($ar) {
				...
				// Вытаскиваем дополнительные параметры для отображения
				$arSelect =["ID", "NAME", "PREVIEW_PICTURE", "DETAIL_PICTURE"];
				$arFilter = ["ID" => $ar["ITEM_ID"]];
				...
				while($ob = $res->GetNextElement())
				{
					...
					$_temp =
						((!empty($arFields["PREVIEW_PICTURE"])) ? $arFields["PREVIEW_PICTURE"]
							: (!empty($arFields["DETAIL_PICTURE"]))) ? $arFields["DETAIL_PICTURE"] : false;

					if($_temp) {
						$ar["PICTURE"] = CFile::ResizeImageGet(
							$_temp,
